...
const AboutPage = () => {
  ...
  const skillItems = skills.map(skill => (
    <div key={skill.name} className={`rounded p-2 text-sm cursor-pointer bg-black text-white hover:bg-white hover:text-black ${skill.name.toLowerCase().replace(/\s+/g, '-')}`}>
      {skill.name}
    </div>
  ));
  return (
        ...
        <div className="p-4 sm:p-8 md:p-12 lg:p-20 xl:p-48 flex flex-col gap-24 md:gap-32 lg:gap-48 xl:gap-64">
          ...
          <div className="flex flex-col gap-12 justify-center">
            ...
            <div className="flex gap-4 flex-wrap">
              {skills.map(skill => (
                <div key={skill.name} className={"rounded p-2 text-sm cursor-pointer bg-black text-white hover:bg-white hover:text-black"}>
                  {skill.name}
                </div>
              ))}
            </div>
